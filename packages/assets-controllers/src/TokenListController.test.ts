...
describe('TokenListController', () => {
  ...
  it('updates tokenList state when network updates are passed via onNetworkStateChange callback', async () => {
  ...
  it('updates the cache before threshold time if the current data is undefined', async () => {
    nock(tokenService.TOKEN_END_POINT_API)
      .get(getTokensPath(ChainId.mainnet))
      .once()
      .reply(200, undefined);
  ...
  it('updates token list from cache before reaching the threshold time', async () => {
    const controllerMessenger = getControllerMessenger();
    const messenger = getRestrictedMessenger(controllerMessenger);
    const controller = new TokenListController({
      chainId: ChainId.mainnet,
  ...
  it('updates token list when the token property changes', async () => {
    nock(tokenService.TOKEN_END_POINT_API)
      .get(getTokensPath(ChainId.mainnet))
      .reply(200, sampleMainnetTokenList)
      .persist();
  ...
  it('updates the cache when the timestamp expires', async () => {
    nock(tokenService.TOKEN_END_POINT_API)
      .get(getTokensPath(ChainId.mainnet))
      .reply(200, sampleMainnetTokenList)
      .persist();
  ...
  it('updates preventPollingOnNetworkRestart and restarts the polling on network restart', async () => {
    nock(tokenService.TOKEN_END_POINT_API)
      .get(getTokensPath(ChainId.mainnet))
      .reply(200, sampleMainnetTokenList)
      .get(getTokensPath(ChainId.goerli))
