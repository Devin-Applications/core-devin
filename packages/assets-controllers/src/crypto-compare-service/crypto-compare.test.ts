...
describe('CryptoCompare', () => {
  it('returns CAD conversion rate', async () => {
  ...
  it('returns CAD conversion rate given lower-cased currency', async () => {
    nock(cryptoCompareHost)
      .get('/data/price?fsym=ETH&tsyms=CAD')
      .reply(200, { CAD: 2000.42 });
  ...
  it('returns CAD conversion rate given lower-cased native currency', async () => {
    nock(cryptoCompareHost)
      .get('/data/price?fsym=ETH&tsyms=CAD')
      .reply(200, { CAD: 2000.42 });
  ...
  it('does not return USD conversion rate when fetching just CAD conversion rate', async () => {
    nock(cryptoCompareHost)
      .get('/data/price?fsym=ETH&tsyms=CAD')
  ...
  it('returns USD conversion rate for USD even when includeUSD is disabled', async () => {
    nock(cryptoCompareHost)
  ...
  it('returns USD conversion rate for USD when includeUSD is enabled', async () => {
    nock(cryptoCompareHost)
      .get('/data/price?fsym=ETH&tsyms=USD')
      .reply(200, { USD: 1000.42 });
  ...
  it('returns CAD and USD conversion rate', async () => {
    nock(cryptoCompareHost)
  ...
  it('throws if fetch returns unsuccessful response', async () => {
  ...
  it('throws if conversion rate is invalid', async () => {
    nock(cryptoCompareHost)
      .get('/data/price?fsym=ETH&tsyms=CAD')
  ...
  it('throws if USD conversion rate is invalid', async () => {
    nock(cryptoCompareHost)
      .get('/data/price?fsym=ETH&tsyms=CAD,USD')
  ...
  it('throws an error if either currency is invalid', async () => {
    nock(cryptoCompareHost)
  ...
  it('overrides native symbol when the CryptoCompare identifier is different', async () => {
    nock(cryptoCompareHost)
  ...
  describe('fetchMultiExchangeRate', () => {
    it('returns CAD and USD conversion rate for BTC, ETH, and SOL', async () => {
      nock(cryptoCompareHost)
    ...
    it('does not return USD value if not requested', async () => {
      nock(cryptoCompareHost)
