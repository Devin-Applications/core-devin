...
describe('BaseController', () => {
  ...
  it('allows getting state via the getState action', () => {
  ...
  it('does not allow reassigning the `state` property', () => {
    const controller = new CountController({
      messenger: getCountMessenger(),
      name: 'CountController',
      state: { count: 0 },
  ...
  it('does not allow reassigning an object property that exists in state', () => {
  ...
  it('does not allow pushing a value onto an array property that exists in state', () => {
    const controller = new MessagesController({
      messenger: getMessagesMessenger(),
      name: messagesControllerName,
      state: {
  ...
  it('allows updating state by returning a value', () => {
    const controller = new CountController({
      messenger: getCountMessenger(),
      name: 'CountController',
      state: { count: 0 },
  ...
  it('does not inform a subscriber of state changes if the selected value is unchanged', () => {
    const controllerMessenger = new ControllerMessenger<
      never,
      CountControllerEvent
    >();
  ...
  it('informs a subscriber of each state change once even after multiple subscriptions', () => {
    const controllerMessenger = new ControllerMessenger<
      never,
      CountControllerEvent
    >();
  ...
  it('no longer informs a subscriber about state changes after unsubscribing', () => {
    const controllerMessenger = new ControllerMessenger<
      never,
      CountControllerEvent
    >();
  ...
  it('no longer informs a subscriber about state changes after unsubscribing once, even if they subscribed many times', () => {
    const controllerMessenger = new ControllerMessenger<
      never,
      CountControllerEvent
    >();
