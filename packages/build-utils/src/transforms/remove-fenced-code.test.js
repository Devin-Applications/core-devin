...
const __1 = require("..");
const remove_fenced_code_1 = require("./remove-fenced-code");
...
const getFencedCode = (...params) => `///: BEGIN:ONLY_INCLUDE_IF(${params.join(',')})
...
describe('build transforms', () => {
    describe('removeFencedCode', () => {
        ...
        it('transforms file consisting of single fence pair', () => {
            expect((0, __1.removeFencedCode)(mockFileName, getFencedCode(FEATURE_A), getFeatures({
        ...
        [
            [
                join(getFencedCode(FEATURE_A), getUnfencedCode(), getFencedCode(FEATURE_C)),
                join('', getUnfencedCode(), ''),
            ],
            [
                join(getFencedCode(FEATURE_A), getFencedCode(FEATURE_B), getFencedCode(FEATURE_C)),
                join('', getFencedCode(FEATURE_B), ''),
            ],
            [
                join(getFencedCode(FEATURE_A), getUnfencedCode(), getFencedCode(FEATURE_B), getFencedCode(FEATURE_C), getUnfencedCode(), getFencedCode(FEATURE_A), getFencedCode(FEATURE_B)),
                join('', getUnfencedCode(), getFencedCode(FEATURE_B), '', getUnfencedCode(), '', getFencedCode(FEATURE_B)),
            ],
            [
                join(getUnfencedCode(), getFencedCode(FEATURE_A), getFencedCode(FEATURE_B), getFencedCode(FEATURE_B), getFencedCode(FEATURE_C), getFencedCode(FEATURE_C), getUnfencedCode(), getFencedCode(FEATURE_A), getFencedCode(FEATURE_A), getFencedCode(FEATURE_B)),
                join(getUnfencedCode(), '', getFencedCode(FEATURE_B), getFencedCode(FEATURE_B), '', '', getUnfencedCode(), '', '', getFencedCode(FEATURE_B)),
            ],
        ].forEach(([input, expected], i) => {
            it(`removes multiple fences from file ${i}`, () => {
                expect((0, __1.removeFencedCode)(mockFileName, input, getFeatures({
                    active: new Set([FEATURE_B]),
        ...
        [
            [
                [FEATURE_A],
                join(getFencedCode(FEATURE_A, FEATURE_B), getUnfencedCode(), getFencedCode(FEATURE_C)),
                join(getFencedCode(FEATURE_A, FEATURE_B), getUnfencedCode(), ''),
                true,
            ],
            [
                [FEATURE_A],
                join(getFencedCode(FEATURE_A, FEATURE_B), getUnfencedCode(), getFencedCode(FEATURE_C, FEATURE_B)),
                join(getFencedCode(FEATURE_A, FEATURE_B), getUnfencedCode(), ''),
                true,
            ],
            [
                [FEATURE_B],
                join(getFencedCode(FEATURE_A, FEATURE_B, FEATURE_C), getUnfencedCode(), getFencedCode(FEATURE_C)),
                join(getFencedCode(FEATURE_A, FEATURE_B, FEATURE_C), getUnfencedCode(), ''),
                true,
            ],
            [
                [FEATURE_B],
                join(getFencedCode(FEATURE_A, FEATURE_B, FEATURE_C), getUnfencedCode(), getFencedCode(FEATURE_C, FEATURE_A)),
                join(getFencedCode(FEATURE_A, FEATURE_B, FEATURE_C), getUnfencedCode(), ''),
                true,
            ],
            [
        ...
        it('ignores sentinels preceded by non-whitespace', () => {
            ...
            ignoredLines.forEach((ignoredLine) => {
                ...
                expect((0, __1.removeFencedCode)(mockFileName, getFencedCode(FEATURE_A).concat(ignoredLine), getFeatures({
                    active: new Set([FEATURE_B]),
                    all: new Set([FEATURE_B, FEATURE_A]),
                }))).toStrictEqual([ignoredLine, true]);
                const modifiedInputWithoutFences = getUnfencedCode().concat(ignoredLine);
                // These inputs will not be transformed
                expect((0, __1.removeFencedCode)(mockFileName, modifiedInputWithoutFences, getFeatures({
                    active: new Set([FEATURE_B]),
        ...
        it('rejects sentinels not followed by a single space and a multi-character alphabetical string', () => {
            ...
            replacements.forEach((replacement) => {
                expect(() => (0, __1.removeFencedCode)(mockFileName, getFencedCode(FEATURE_B).replace(fenceSentinelAndTerminusRegex, replacement), getFeatures({
                    active: new Set([FEATURE_B]),
                    all: new Set([FEATURE_B]),
                }))).toThrow(/Fence sentinel must be followed by a single space and an alphabetical string of two or more characters.$/u);
            });
        });
        it('rejects malformed BEGIN directives', () => {
            ...
            replacements.forEach((replacement) => {
                expect(() => (0, __1.removeFencedCode)(mockFileName, getFencedCode(FEATURE_B).replace(directiveString, replacement), getFeatures({
                    active: new Set([FEATURE_B]),
                    all: new Set([FEATURE_B]),
                }))).toThrow(new RegExp(`${replacement.replace(/([()[\]])/gu, '\\$1')}":\nFailed to parse fence directive.$`, 'u'));
            });
        });
        it('rejects malformed END directives', () => {
            ...
            replacements.forEach((replacement) => {
                expect(() => (0, __1.removeFencedCode)(mockFileName, getFencedCode(FEATURE_B).replace(directiveString, replacement), getFeatures({
                    active: new Set([FEATURE_B]),
                    all: new Set([FEATURE_B]),
                }))).toThrow(new RegExp(`${replacement}":\nFailed to parse fence directive.$`, 'u'));
            });
        });
        it('rejects files with uneven number of fence lines', () => {
            ...
            additions.forEach((addition) => {
                expect(() => (0, __1.removeFencedCode)(mockFileName, getFencedCode(FEATURE_B).concat(addition), getFeatures({
                    active: new Set([FEATURE_B]),
                    all: new Set([FEATURE_B]),
                }))).toThrow(/A valid fence consists of two fence lines, but the file contains an uneven number, "3", of fence lines.$/u);
            });
        });
        it('rejects invalid terminuses', () => {
            ...
            testCases.forEach(([validTerminus, replacements]) => {
                replacements.forEach((replacement) => {
                    expect(() => (0, __1.removeFencedCode)(mockFileName, getFencedCode(FEATURE_B).replace(validTerminus, replacement), getFeatures({
                        active: new Set([FEATURE_B]),
                        all: new Set([FEATURE_B]),
                    }))).toThrow(new RegExp(`Line contains invalid directive terminus "${replacement}".$`, 'u'));
                });
            });
        });
        it('rejects invalid commands', () => {
            ...
            testCases.forEach(([validCommand, replacements]) => {
                replacements.forEach((replacement) => {
                    expect(() => (0, __1.removeFencedCode)(mockFileName, getFencedCode(FEATURE_B).replace(validCommand, replacement), getFeatures({
                        active: new Set([FEATURE_B]),
                        all: new Set([FEATURE_B]),
                    }))).toThrow(new RegExp(`Line contains invalid directive command "${replacement.replace('(', '')}".$`, 'u'));
                });
            });
        });
        it('rejects invalid command parameters', () => {
            ...
            testCases.forEach(([invalidParam, replacements]) => {
                replacements.forEach((replacement) => {
                    expect(() => (0, __1.removeFencedCode)(mockFileName, getFencedCode(replacement), getFeatures({
                        active: new Set([FEATURE_B]),
                        all: new Set([FEATURE_B, FEATURE_A, FEATURE_C]),
                    }))).toThrow(new RegExp(`"${invalidParam}" is not a declared build feature.$`, 'u'));
                });
            });
            // Should fail for empty params
            expect(() => (0, __1.removeFencedCode)(mockFileName, getFencedCode('').replace('()', ''), getFeatures({
                active: new Set([FEATURE_B]),
                all: new Set([FEATURE_B]),
            }))).toThrow('Invalid code fence parameters in file "file.js":\nNo parameters specified.');
        });
        it('rejects directive pairs with wrong terminus order', () => {
            ...
            const expectedBeginError = 'The first directive of a pair must be a "BEGIN" directive.';
            const expectedEndError = 'The second directive of a pair must be an "END" directive.';
            const testCases = [
                [
                    'BEGIN:ONLY_INCLUDE_IF(feature-b)',
                    'END:ONLY_INCLUDE_IF',
            ...
            testCases.forEach(([target, replacement, expectedError]) => {
                expect(() => (0, __1.removeFencedCode)(mockFileName, input.replace(target, replacement), getFeatures({
                    active: new Set([FEATURE_B]),
                    all: new Set([FEATURE_B]),
    ...
    describe('multiSplice', () => {
        it('throws if the indices array is empty or of odd length', () => {
            [[], [1], [1, 2, 3]].forEach((invalidInput) => {
                expect(() => (0, remove_fenced_code_1.multiSplice)('foobar', invalidInput)).toThrow('Expected non-empty, even-length array.');
            });
        });
        it('throws if the indices array contains non-integer or negative numbers', () => {
            [
                ...
                expect(() => (0, remove_fenced_code_1.multiSplice)('foobar', invalidInput)).toThrow('Expected array of non-negative integers.');
            });
        });
    });
    describe('validateCommand', () => {
        it('throws if the parameters are invalid', () => {
            [null, undefined, []].forEach((invalidInput) => {
                expect(() => (0, remove_fenced_code_1.validateCommand)(remove_fenced_code_1.DirectiveCommand.ONLY_INCLUDE_IF, 
                // TODO: Replace `any` with type
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                invalidInput, 'file.js', 
                // TODO: Replace `any` with type
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
        ...
        it('throws if the command is unrecognized', () => {
            // TODO: Replace `any` with type
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            expect(() => (0, remove_fenced_code_1.validateCommand)('foobar', [], 'file.js', {})).toThrow('Unrecognized command "foobar".');
        });
    });
});
