...
describe('JsonRpcEngine', () => {
    it('handle: throws on truthy, non-function callback', () => {
        ...
        expect(() => engine.handle({}, 'foo')).toThrow('"callback" must be a function if provided.');
    });
    ...
    it('handle: returns error for invalid request method', async () => {
        ...
        // TODO: Replace `any` with type
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const response = await engine.handle({ id: 1, method: null });
        expect(response.error.code).toBe(-32600);
        expect(response.result).toBeUndefined();
    });
    ...
    it('handle: returns undefined for malformed notifications', async () => {
        ...
        const engine = new _1.JsonRpcEngine({ notificationHandler });
        engine.push(middleware);
        expect(
        // TODO: Replace `any` with type
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        await engine.handle({ jsonrpc, method: true })).toBeUndefined();
    ...
    it('handle: re-throws errors from notification handlers (async)', async () => {
        ...
        const engine = new _1.JsonRpcEngine({ notificationHandler });
        await expect(engine.handle({ jsonrpc, method: 'foo' })).rejects.toThrow(new Error('baz'));
        expect(notificationHandler).toHaveBeenCalledTimes(1);
        expect(notificationHandler).toHaveBeenCalledWith({
            jsonrpc,
            method: 'foo',
    ...
    it('interacting middleware test', async () => {
        ...
        // TODO: Replace `any` with type
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        engine.push(function (request, _response, next, _end) {
    ...
    it('erroring middleware test: end(error)', async () => {
        ...
        await new Promise((resolve) => {
            engine.handle(payload, function (error, response) {
                ...
                (0, utils_1.assertIsJsonRpcFailure)(response);
    ...
    it('erroring middleware test: response.error -> next()', async () => {
        ...
        engine.push(function (_request, response, next, _end) {
            response.error = rpc_errors_1.rpcErrors.internal({ message: 'foobar' });
            next();
        });
        const payload = { id: 1, jsonrpc, method: 'hello' };
        await new Promise((resolve) => {
    ...
    it('erroring middleware test: response.error -> end()', async () => {
        ...
        await new Promise((resolve) => {
            engine.handle(payload, function (error, response) {
                expect(error).toBeDefined();
                expect(response).toBeDefined();
                expect((0, utils_1.isJsonRpcFailure)(response)).toBe(true);
                expect((0, utils_1.isJsonRpcSuccess)(response)).toBe(false);
                resolve();
            });
        });
    ...
    it('erroring middleware test: non-function passsed to next()', async () => {
        ...
        await new Promise((resolve) => {
            engine.handle(payload, function (error, response) {
                ...
                expect(response.error.message.startsWith('JsonRpcEngine: "next" return handlers must be functions.')).toBe(true);
                expect((0, utils_1.isJsonRpcSuccess)(response)).toBe(false);
                resolve();
            });
        });
    ...
    it('handle: empty batch', async () => {
        ...
        await new Promise((resolve) => {
            // TODO: Replace `any` with type
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            engine.handle(emptyBatch, function (error, response) {
                expect(error).toBeNull();
                expect(response).toBeInstanceOf(Array);
                expect(response).toHaveLength(1);
                expect(response[0].error.message.startsWith('Request batch must contain plain objects. Received an empty array')).toBe(true);
                expect((0, utils_1.isJsonRpcSuccess)(response[0])).toBe(false);
                resolve();
            });
        });
    });
    it('handle: empty batch (async signature)', async () => {
        ...
        // TODO: Replace `any` with type
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const response = await engine.handle(emptyBatch);
        expect(response).toBeInstanceOf(Array);
        expect(response).toHaveLength(1);
        expect(response[0].error.message.startsWith('Request batch must contain plain objects. Received an empty array')).toBe(true);
        expect((0, utils_1.isJsonRpcSuccess)(response[0])).toBe(false);
    ...
    it('handle: batch payloads', async () => {
        ...
        await new Promise((resolve) => {
            ...
            engine.handle(payload, function (error, response) {
                ...
                expect((0, utils_1.isJsonRpcSuccess)(response[3])).toBe(false);
    ...
    it('handle: batch payload with bad request object', async () => {
        ...
        expect(response).toBeInstanceOf(Array);
        expect(response[0].result).toBe(1);
        expect((0, utils_1.isJsonRpcSuccess)(response[1])).toBe(false);
    ...
    it('handles failure to end request', async () => {
        ...
        await new Promise((resolve) => {
            // TODO: Replace `any` with type
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            engine.handle(payload, (error, response) => {
                expect(error.message.startsWith('JsonRpcEngine: Nothing ended request:')).toBe(true);
                expect((0, utils_1.isJsonRpcSuccess)(response)).toBe(false);
                resolve();
            });
        });
    ...
    it('handles batch request processing error (async)', async () => {
        ...
        // TODO: Replace `any` with type
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        await expect(engine.handle([{}])).rejects.toThrow('foo');
    });
    describe('destroy', () => {
        const destroyedError = new Error('This engine is destroyed and can no longer be used.');
        it('prevents the engine from being used', async () => {
            const engine = new _1.JsonRpcEngine();
            engine.destroy();
            await expect(async () => engine.handle([])).rejects.toThrow(destroyedError);
        ...
        it('calls the destroy method of middleware functions', async () => {
            ...
            const destroyableMiddleware = (_request, _response, _next, end) => {
                end();
            };
